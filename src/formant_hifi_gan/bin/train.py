...
import formant_hifi_gan
import formant_hifi_gan.models
import hydra
...
class Trainer(object):
    ...
    def _train_step(self, batch):
        ...
        if self.config.train.lambda_reg > 0:
            s = outs[1]
            if isinstance(self.criterion["reg"], formant_hifi_gan.losses.ResidualLoss):
        ...
        for train_steps_per_epoch, batch in enumerate(self.data_loader["train"], 1):
            ...
            if self.finish_train:
                self.save_checkpoint(
                    os.path.join(self.config.out_dir, "checkpoints", f"checkpoint-{self.steps}steps.pkl")
                )
    ...
    def _eval_step(self, batch):
        ...
        if self.config.train.lambda_reg > 0:
            ...
            if isinstance(
                self.criterion["reg"],
                formant_hifi_gan.losses.ResidualLoss,
            ):
                reg_loss = self.criterion["reg"](s, y, f0)
                gen_loss += self.config.train.lambda_reg * reg_loss
...
class Collator(object):
    ...
    def __call__(self, batch):
        ...
        for idx in range(len(batch)):
            x, c, f0, cf0 = batch[idx]  # audio, aux_feats, f0, cf0
